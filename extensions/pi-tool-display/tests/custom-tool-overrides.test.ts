...
function createNativeToolView(tool: RuntimeTool, config: ToolDisplayConfig): RuntimeTool {
	...
	return {
		...
		renderCall: patch.createCallRenderer(instance) as RuntimeTool["renderCall"],
		renderResult: patch.createResultRenderer(instance) as RuntimeTool["renderResult"],
	};
}
...
test("enabled generic custom tool override replaces existing extension renderers and leaves other tools alone", async () => {
	...
	const enabledView = createNativeToolView(enabledTool, config);
	const disabledView = createNativeToolView(disabledTool, config);
	const unlistedView = createNativeToolView(unlistedTool, config);

	assert.equal(renderToText(enabledView.renderCall?.({ query: "Widget", limit: 5 }, createTheme())), "ide_find_symbol (2 args)");
	assert.equal(renderToolResult(enabledView, "alpha\nbeta\ngamma\n"), "↳ 3 lines returned • Ctrl+O to expand");
	assert.equal(renderToText(disabledView.renderCall?.({}, createTheme())), "RAW DISABLED CALL");
	assert.equal(renderToolResult(disabledView, "ignored"), "RAW DISABLED RESULT");
	assert.equal(renderToText(unlistedView.renderCall?.({}, createTheme())), "RAW UNLISTED CALL");
	assert.equal(renderToolResult(unlistedView, "ignored"), "RAW UNLISTED RESULT");
});
...
test("custom tool override defaults kind to generic unless the user chooses mcp", async () => {
	...
	registerToolDisplayOverrides(api, () => config);
	await runLifecycle(eventHandlers);
	const genericView = createNativeToolView(genericTool, config);
	const mcpView = createNativeToolView(mcpTool, config);

	assert.equal(renderToText(genericView.renderCall?.({ tool: "read_file", server: "filesystem" }, createTheme())), "remote_gateway (2 args)");
	assert.equal(renderToText(mcpView.renderCall?.({ tool: "read_file", server: "filesystem" }, createTheme())), "MCP call filesystem:read_file (no args)");
});
...
test("custom generic tool override honors per-tool hidden output mode", async () => {
	...
	assert.equal(renderToolResult(createNativeToolView(quietTool, config), "secret\nnoisy\noutput\n"), "");
});
...
test("generic custom tool renderCall handles absent, non-object, and nested arguments safely", async () => {
	...
	registerToolDisplayOverrides(api, () => config);
	await runLifecycle(eventHandlers);
	const argumentView = createNativeToolView(argumentProbe, config);

	assert.equal(renderToText(argumentView.renderCall?.(undefined, createTheme())), "argument_probe (no args)");
	assert.equal(renderToText(argumentView.renderCall?.(null, createTheme())), "argument_probe (no args)");
	assert.equal(renderToText(argumentView.renderCall?.("raw string args", createTheme())), "argument_probe (no args)");
	assert.equal(renderToText(argumentView.renderCall?.(["array", "args"], createTheme())), "argument_probe (no args)");
	assert.equal(
		renderToText(argumentView.renderCall?.({ path: "src/index.ts", options: { recursive: true }, tags: ["a", "b"] }, createTheme())),
...
test("generic custom tool preview mode supports collapsed previews, expanded previews, partial state, and empty text", async () => {
	...
	registerToolDisplayOverrides(api, () => config);
	await runLifecycle(eventHandlers);
	const previewView = createNativeToolView(previewTool, config);

	assert.equal(
		renderToolResult(previewView, "alpha\nbeta\ngamma\ndelta\n"),
	...
	assert.equal(renderToolResult(previewView, "still running", { isPartial: true }), "running...");
	assert.equal(
		renderToolRawResult(previewView, { content: [{ type: "image", data: "ignored" }], details: {} }),
		"↳ (no output)",
	);
	assert.equal(renderToolRawResult(previewView, { details: {} }), "↳ (no output)");
});
...
test("explicit mcp custom tool override interprets MCP proxy argument variants", async () => {
	...
	registerToolDisplayOverrides(api, () => config);
	await runLifecycle(eventHandlers);
	const customMcpView = createNativeToolView(customMcpProxy, config);

	assert.equal(renderToText(customMcpView.renderCall?.({}, createTheme())), "MCP status");
	assert.equal(renderToText(customMcpView.renderCall?.({ connect: "filesystem" }, createTheme())), "MCP connect filesystem");
	assert.equal(renderToText(customMcpView.renderCall?.({ describe: "read_file", server: "filesystem" }, createTheme())), "MCP describe read_file @ filesystem");
	assert.equal(renderToText(customMcpView.renderCall?.({ search: "browser", server: "exa" }, createTheme())), "MCP search \"browser\" @ exa");
	assert.equal(renderToText(customMcpView.renderCall?.({ server: "filesystem" }, createTheme())), "MCP tools filesystem");
	assert.equal(renderToText(customMcpView.renderCall?.({ tool: "read_file", server: "filesystem" }, createTheme())), "MCP call filesystem:read_file (no args)");
});
...
test("custom tool override preserves execution contract, parameters, and prepareArguments", async () => {
	...
	registerToolDisplayOverrides(api, () => config);
	await runLifecycle(eventHandlers);
	const contractView = createNativeToolView(contractTool, config);
	...
	assert.equal(typeof contractView.renderCall, "function");
	assert.equal(typeof contractView.renderResult, "function");
});
...
test("custom tool registered after lifecycle is decorated when it is explicitly opted in", async () => {
	...
	const lateView = createNativeToolView(lateTool, config);

	assert.equal(typeof lateView.renderCall, "function");
	assert.equal(typeof lateView.renderResult, "function");
	assert.equal(renderToText(lateView.renderCall?.({ query: "late" }, createTheme())), "late_custom_tool (1 arg)");
});
