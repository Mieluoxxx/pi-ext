...
import { applyToolDisplaySetting, registerToolDisplayCommand } from "../src/config-modal.ts";
import {
...
test("'show' argument notifies with config summary", async () => {
	...
	assert.ok(notifications[0]?.message.includes("bashChars=2000"));
...
test("settings apply the Bash collapsed character budget", () => {
	const next = applyToolDisplaySetting(
		DEFAULT_TOOL_DISPLAY_CONFIG,
		"bashCollapsedMaxChars",
		"4000",
	);
	assert.equal(next.bashCollapsedMaxChars, 4000);
});
