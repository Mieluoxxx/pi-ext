...
test("4: MCP tools use the native execution renderer patch immediately", () => {
  ...
  const { api } = createExtensionApiStub([mcpTool]);
  const prototype = ToolExecutionComponent.prototype as unknown as PatchableToolExecutionPrototype;
  registerToolDisplayOverrides(api, () => DEFAULT_TOOL_DISPLAY_CONFIG);
  const instance = { toolName: "weather", toolDefinition: mcpTool };

  assert.equal(prototype.getRenderShell.call(instance), "default");
  assert.equal(typeof prototype.getCallRenderer.call(instance), "function");
  assert.equal(typeof prototype.getResultRenderer.call(instance), "function");
  assert.equal(mcpTool.renderShell, "self", "adapter definition remains untouched");
  assert.equal(typeof mcpTool.renderCall, "undefined");
});

test("4: MCP execution renderer patch stays idempotent across reload", () => {
  const firstDefinition: Record<string, unknown> = {
    name: "weather",
    description: "MCP weather tool for forecasts",
    parameters: { type: "object", properties: {} },
    renderShell: "self",
  ...
  const prototype = ToolExecutionComponent.prototype as unknown as PatchableToolExecutionPrototype;
  const { api: api1 } = createExtensionApiStub([firstDefinition]);
  registerToolDisplayOverrides(api1, () => DEFAULT_TOOL_DISPLAY_CONFIG);
  const patchedGetRenderShell = prototype.getRenderShell;
  const replacementDefinition = { ...firstDefinition };
  const { api: api2 } = createExtensionApiStub([replacementDefinition]);
  registerToolDisplayOverrides(api2, () => DEFAULT_TOOL_DISPLAY_CONFIG);

  assert.equal(prototype.getRenderShell, patchedGetRenderShell, "reload updates the patch instead of wrapping twice");
  assert.equal(
    prototype.getRenderShell.call({ toolName: "weather", toolDefinition: replacementDefinition }),
    "default",
  );
  assert.equal(typeof prototype.getCallRenderer.call({ toolName: "weather", toolDefinition: replacementDefinition }), "function");
});
...
test("11: registerToolDisplayOverrides refreshes native patch config on each call", () => {
  const prototype = ToolExecutionComponent.prototype as unknown as PatchableToolExecutionPrototype;
  const definition = {
    name: "custom_gateway",
    description: "Custom gateway without protocol metadata.",
    renderShell: "self",
  };
  const instance = { toolName: "custom_gateway", toolDefinition: definition };
  const enabledConfig = {
    ...DEFAULT_TOOL_DISPLAY_CONFIG,
    customToolOverrides: {
      custom_gateway: { enabled: true, kind: "mcp" as const, outputMode: "summary" as const },
    },
  };
  const disabledConfig = {
    ...DEFAULT_TOOL_DISPLAY_CONFIG,
    customToolOverrides: {
      custom_gateway: { enabled: false, kind: "mcp" as const, outputMode: "summary" as const },
    },
  };

  registerToolDisplayOverrides(createExtensionApiStub().api, () => enabledConfig);
  assert.equal(prototype.getRenderShell.call(instance), "default");

  registerToolDisplayOverrides(createExtensionApiStub().api, () => disabledConfig);
  assert.equal(prototype.getRenderShell.call(instance), "self");
});
...
test("12: config-store reloads config on fingerprint change between calls", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-tool-display-config-reload-"));
  const configDir = join(agentDir, "extensions", "pi-tool-display");
  ...
  writeFileSync(
    configFile,
    JSON.stringify({ readOutputMode: "hidden", searchOutputMode: "hidden" }),
    "utf8",
  );
  process.env.PI_CODING_AGENT_DIR = agentDir;

  try {
    const configStoreUrl = new URL("../src/config-store.ts?reload-behavior-test", import.meta.url);
    const { loadToolDisplayConfig, saveToolDisplayConfig } = await import(configStoreUrl.href);
    ...
    assert.equal(initialResult.config.searchOutputMode, "hidden");

    writeFileSync(
      configFile,
      JSON.stringify({ readOutputMode: "preview", searchOutputMode: "count", previewLines: 12 }),
      "utf8",
    );
    const changedResult = loadToolDisplayConfig();
    assert.equal(changedResult.config.readOutputMode, "preview");
    assert.equal(changedResult.config.searchOutputMode, "count");

    const saveResult = saveToolDisplayConfig(changedResult.config);
