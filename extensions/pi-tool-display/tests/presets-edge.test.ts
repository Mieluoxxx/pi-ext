...
test("getToolDisplayPresetConfig returns correct config for opencode preset", () => {
	...
	assert.equal(config.bashCollapsedMaxChars, 2000);
...
test("detectToolDisplayPreset returns 'custom' when bashCollapsedMaxChars differs", () => {
	const verbose = getToolDisplayPresetConfig("verbose");
	const modified = { ...verbose, bashCollapsedMaxChars: verbose.bashCollapsedMaxChars + 1 };
	assert.equal(detectToolDisplayPreset(modified), "custom");
});
