...
test("pending preview skips direct reads outside the workspace", () => {
  withTempWorkspace("pi-tool-display-boundary-", (workspace) => {
    ...
    try {
      ...
      const traversalPreview = buildPendingEditPreviewData(
        { path: relative(workspace, outsideFile), oldText: "secret", newText: "safe" },
        workspace,
      );
      ...
      assert.match(traversalPreview?.notice ?? "", /outside the current workspace/);

      try {
        const linkedFile = join(workspace, "linked-secret.txt");
        symlinkSync(outsideFile, linkedFile);
        const linkedPreview = buildPendingEditPreviewData(
          { path: "linked-secret.txt", oldText: "secret", newText: "safe" },
          workspace,
        );
        assert.match(linkedPreview?.notice ?? "", /resolves outside the current workspace/);
      } catch (error) {
