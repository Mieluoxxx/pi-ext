...
[![npm version](https://img.shields.io/npm/v/%40moguw%2Fpi-tool-display?style=for-the-badge)](https://www.npmjs.com/package/@moguw/pi-tool-display)
[![License](https://img.shields.io/github/license/Mieluoxxx/pi-ext?style=for-the-badge)](LICENSE)
...
- **Adapter API for renderer consumers** through the `@moguw/pi-tool-display/tool-display-api-consumer` subpath export
...
```bash
pi install npm:@moguw/pi-tool-display
```

Restart Pi or run `/reload` after installation, then open `/tool-display` to configure the extension.
...
The Git source installs every extension in the monorepo. Use the scoped npm package when you only need `pi-tool-display`.
...
```ts
import { decorateToolForDisplay, decorateMcpToolForDisplay } from "@moguw/pi-tool-display/tool-display-api-consumer";
```
...
| Mode | Behavior |
|------|----------|
| `opencode` | Classic collapsed output using `bashCollapsedLines` and `bashCollapsedMaxChars` limits with expansion hint |
...
| `bashCollapsedMaxChars` | number | `2000` | Maximum visible characters for unexpanded bash output; `0` disables the character limit |
...
When MCP tooling is available, the extension gives proxy calls, `mcpScript`, and direct MCP wrappers the same call-line, default-shell, preview, error, and expansion behavior as other decorated tools. Collapsed calls show one semantic line and count the nested MCP tool arguments instead of the proxy envelope; expanding reveals the structured arguments.

`mcpOutputMode`, `previewLines`, and `expandedPreviewMaxLines` control MCP results. While `pi-tool-display` owns the renderer, adapter-specific presentation settings such as `toolResultRendering` and `collapsedResultLines` are ignored; MCP execution and transport remain adapter-owned.
...
MCP tools are decorated at Pi's native `ToolExecutionComponent` rendering boundary. This avoids mutating the metadata copies returned by `getAllTools()` and works regardless of which extension registered or later replaced the tool. The proxy, `mcpScript`, and adapter labels beginning with `MCP:` are detected automatically.
...
│   ├── mcp-tool-execution-patch.ts  # Reload-safe native MCP/custom renderer patch
...
`@moguw/pi-tool-display` is based on [pi-tool-display](https://github.com/MasuRii/pi-tool-display), originally created by [MasuRii](https://github.com/MasuRii). Thanks to MasuRii and the upstream contributors for the original extension and its history.
