...
### Added
- Added `bashCollapsedMaxChars` to bound collapsed and partial Bash output by ANSI-safe Unicode character count while leaving expanded output unchanged.
...
- Resolved relative pending-preview paths against the canonical workspace root so macOS `/var` aliases do not produce false outside-workspace notices.
