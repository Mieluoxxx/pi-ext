...
} from "./mcp-tool-execution-patch.js";
...
const SGR_SEQUENCE_PREFIX = /^\x1b\[[0-9;]*m/;
const SGR_STYLE_RESET = `\x1b[${STYLE_RESET_PARAMS.join(";")}m`;

type BashCollapsedPreview = {
  lines: string[];
  remainingCharacters: number;
  remainingLines: number;
};

function countVisibleCodePoints(text: string): number {
  return Array.from(text.replace(ANSI_SGR_PATTERN, "")).length;
}

function truncateAnsiToCodePoints(text: string, maxCharacters: number): string {
  let output = "";
  let index = 0;
  let visibleCharacters = 0;
  let sawSgr = false;

  while (index < text.length && visibleCharacters < maxCharacters) {
    const sequence = text.slice(index).match(SGR_SEQUENCE_PREFIX)?.[0];
    if (sequence) {
      output += sequence;
  ...
  return index < text.length && sawSgr ? `${output}${SGR_STYLE_RESET}` : output;
}

function prepareCollapsedBashPreview(
  rawLines: string[],
  maxLines: number,
  maxCharacters: number,
): BashCollapsedPreview {
  const lines = rawLines.map(sanitizeAnsiForThemedOutput);
  const lineLimit = Math.max(0, Math.floor(maxLines));
  const linePreview = lines.slice(0, lineLimit);
  const remainingLines = Math.max(0, lines.length - linePreview.length);

  if (maxCharacters <= 0) {
    return { lines: linePreview, remainingCharacters: 0, remainingLines };
  }

  const previewCharacterCount = linePreview.reduce(
    (total, line) => total + countVisibleCodePoints(line),
    0,
  );
  if (previewCharacterCount <= maxCharacters) {
    return { lines: linePreview, remainingCharacters: 0, remainingLines };
  }
  ...
  for (const line of linePreview) {
    const remainingBudget = maxCharacters - displayedCharacters;
    if (remainingBudget <= 0) break;

    const lineCharacters = countVisibleCodePoints(line);
    if (lineCharacters <= remainingBudget) {
      displayedLines.push(line);
      displayedCharacters += lineCharacters;
      continue;
    }

    displayedLines.push(truncateAnsiToCodePoints(line, remainingBudget));
    displayedCharacters += remainingBudget;
    break;
  }

  const totalCharacters = lines.reduce((total, line) => total + countVisibleCodePoints(line), 0);
  return {
    lines: displayedLines,
    remainingCharacters: Math.max(0, totalCharacters - displayedCharacters),
    remainingLines,
  };
}

function formatBashCollapsedHint(
  remainingLines: number,
  remainingCharacters: number,
  theme: RenderTheme,
): string {
  ...
  if (remainingLines > 0) {
    omissions.push(`${remainingLines} more ${pluralize(remainingLines, "line")}`);
  }
  if (remainingCharacters > 0) {
    omissions.push(`${remainingCharacters} more ${pluralize(remainingCharacters, "character")}`);
  }
  if (omissions.length === 0) return "";
  return `\n${theme.fg("muted", `... (${omissions.join(" • ")} • Ctrl+O to expand)`)}`;
}

function buildCollapsedBashPreviewText(
  lines: string[],
  maxLines: number,
  maxCharacters: number,
  theme: RenderTheme,
): string {
  const preview = prepareCollapsedBashPreview(lines, maxLines, maxCharacters);
  const text = preview.lines
    .map((line) => theme.fg("toolOutput", line))
    .join("\n");
  return text + formatBashCollapsedHint(preview.remainingLines, preview.remainingCharacters, theme);
}
...
function renderBashPreviewWithHints(
  ...
  details: BashToolDetails | undefined,
	...
	if (!options.expanded) {
		let preview = buildCollapsedBashPreviewText(lines, maxLines, config.bashCollapsedMaxChars, theme);
...
interface McpCallPresentation {
  target: string;
  mutedSuffix: string;
  expandedArguments?: unknown;
}
...
function formatArgCountText(argCount: number): string {
  return argCount === 0
    ? " (no args)"
    : ` (${argCount} ${pluralize(argCount, "arg")})`;
}

function parseMcpArguments(value: unknown): ParsedMcpArguments {
  ...
  if (typeof value === "string") {
    try {
      ...
      return { expandedArguments: value };
    }
  }
  ...
  return {
    ...
    expandedArguments: parsed,
  };
}

function omitUndefinedFields(args: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(args).filter(([, value]) => value !== undefined),
  );
}

function formatExpandedMcpArguments(value: unknown): string | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (
...
function getMcpInlineField(args: Record<string, unknown>, field: string): string | undefined {
  const value = getTextField(args, field);
  return value?.replace(/\s+/gu, " ").trim();
}

function resolveMcpProxyCall(args: Record<string, unknown>): McpCallPresentation {
  const tool = getMcpInlineField(args, "tool");
  const connect = getMcpInlineField(args, "connect");
  const describe = getMcpInlineField(args, "describe");
  const instructions = getMcpInlineField(args, "instructions");
  const search = getMcpInlineField(args, "search");
  const server = getMcpInlineField(args, "server");

  if (tool) {
    const parsedArgs = parseMcpArguments(args.args);
    return {
      target: server ? `call ${server}:${tool}` : `call ${tool}`,
      mutedSuffix: parsedArgs.count === undefined
        ? " (args)"
        : formatArgCountText(parsedArgs.count),
      expandedArguments: parsedArgs.expandedArguments,
    };
  }
  if (connect) {
    return {
      ...
      mutedSuffix: "",
      expandedArguments: omitUndefinedFields(args),
    };
  }
  if (describe) {
    return {
      target: server ? `describe ${describe} @ ${server}` : `describe ${describe}`,
      mutedSuffix: "",
      expandedArguments: omitUndefinedFields(args),
    };
  }
  ...
  if (search) {
    ...
    if (server) qualifiers.push(`@ ${server}`);
    if (args.regex === true) qualifiers.push("regex");
    if (args.includeSchemas === false) qualifiers.push("schemas hidden");
    ...
    if (offset !== undefined) qualifiers.push(`offset ${offset}`);
    return {
      target: `search "${search}"`,
      mutedSuffix: qualifiers.length > 0 ? ` ${qualifiers.join(" ")}` : "",
      expandedArguments: omitUndefinedFields(args),
    };
  }
  const action = getMcpInlineField(args, "action");
  if (action) {
    return {
      target: action,
      mutedSuffix: server ? ` @ ${server}` : "",
      expandedArguments: omitUndefinedFields(args),
    };
  }
  if (server) {
...
function formatArgCountSuffix(argCount: number, theme: RenderTheme): string {
  return theme.fg("muted", formatArgCountText(argCount));
}

function normalizeMcpDirectTarget(toolName: string, toolLabel: string): string {
  const normalized = toolLabel
    .replace(/^MCP(?:\s*:\s*|\s+)/iu, "")
    .replace(/\s+/gu, " ")
    .trim();
  return normalized && normalized.toLowerCase() !== "mcp"
    ? normalized
    : toolName;
}
...
function formatMcpCallLine(
  ...
  const presentation = toolName === "mcp"
    ? resolveMcpProxyCall(args)
    : {
        target: normalizeMcpDirectTarget(toolName, toolLabel),
        mutedSuffix: formatArgCountText(Object.keys(args).length),
        expandedArguments: args,
      };
  let text = `${theme.fg("toolTitle", theme.bold("MCP"))} ${theme.fg("accent", presentation.target)}${theme.fg("muted", presentation.mutedSuffix)}`;
  const expandedArguments = context?.expanded
    ? formatExpandedMcpArguments(presentation.expandedArguments)
    : undefined;
  if (expandedArguments) {
    text += `\n${theme.fg("muted", expandedArguments)}`;
  }
  return new Text(text, 0, 0);
}

function extractMcpOutput(result: ToolRenderInput): string {
  ...
  return blocks.map((rawBlock) => {
    ...
    const mimeType = typeof block.mimeType === "string" && block.mimeType.trim()
      ? block.mimeType.trim()
      : undefined;
    return mimeType ? `[${type}: ${mimeType}]` : `[${type}]`;
  }).join("\n");
}
...
function renderMcpResult(
  ...
  theme: RenderTheme,
  context?: ToolRenderContextLike,
  extractOutput: (value: ToolRenderInput) => string = extractMcpOutput,
): Text {
  ...
  const lines = prepareOutputLines(extractOutput(result), options);
  ...
  if (isToolError(result, context) || Boolean(toRecord(result.details).error)) {
    const maxLines = options.expanded
      ? getExpandedPreviewLineLimit(lines, config)
      : config.previewLines;
    const { shown, remaining } = previewLines(lines, maxLines);
    let errorText = theme.fg("error", "↳ MCP call failed");
    if (shown.length > 0) {
      errorText += `\n${shown
        .map((line) => theme.fg("error", sanitizeAnsiForThemedOutput(line)))
        .join("\n")}`;
      errorText += formatTruncationHint(remaining, options.expanded, theme);
    }
    return textResult(appendMcpPreviewHints(errorText, mcpCtx));
  }
...
function renderCustomToolResult(
  ...
  options: ToolRenderResultOptions,
  config: ToolDisplayConfig,
  kind: CustomToolOverrideConfig["kind"],
  ...
  return renderMcpResult(
    ...
    theme,
    context,
    kind === "mcp" ? extractMcpOutput : extractTextOutput,
  );
}
...
function resolveNativeToolDisplayPresentation(
  instance: PatchableToolExecutionInstance,
  config: ToolDisplayConfig,
): NativeToolDisplayPresentation | undefined {
  const toolName = getTextField(instance, "toolName");
  if (!toolName) {
    return undefined;
  }

  const customOverride = getRuntimeCustomToolOverride(toolName, config);
  if (customOverride?.enabled) {
    return {
      kind: customOverride.kind,
      toolName: customOverride.kind === "mcp" ? "mcp" : toolName,
      toolLabel: customOverride.kind === "mcp" ? "MCP Proxy" : toolName,
      customOverride,
    };
  }
  ...
  return {
    kind: "mcp",
    toolName,
    toolLabel: getTextField(definition, "label") || (toolName === "mcp" ? "MCP Proxy" : `MCP ${toolName}`),
  };
}

export function createMcpToolExecutionPatchOptions(
  getConfig: ConfigGetter,
): McpToolExecutionPatchOptions {
  return {
    matches(instance) {
      ...
      return (
        ...
        if (presentation?.customOverride) {
          return renderCustomToolResult(
            ...
            getConfig(),
            presentation.kind,
            presentation.customOverride.outputMode,
            theme,
            context,
          );
        }
        return renderMcpResult(result, options, getConfig(), theme, context);
      };
    },
  };
}
...
function installNativeMcpToolExecutionPatch(getConfig: ConfigGetter): void {
...
function installToolDisplayApi(getConfig: ConfigGetter): ToolDisplayApi {
  ...
  const api: ToolDisplayApi = {
    ...
    decorateTool<T extends RuntimeToolDefinition>(tool: T, adapter?: ToolDisplayAdapter): T {
      ...
      const replaceExisting = overrideExisting || kind === "mcp";
      const decorated: RuntimeToolDefinition = { ...tool };

      if (resolvedAdapter.renderCall && (replaceExisting || typeof decorated.renderCall !== "function")) {
        decorated.renderCall = resolvedAdapter.renderCall;
      } else if (kind === "read" && (replaceExisting || typeof decorated.renderCall !== "function")) {
        decorated.renderCall = (args: unknown, theme: RenderTheme) => renderReadDisplayCall(args, theme, resolvedAdapter);
      } else if (kind === "edit" && (replaceExisting || typeof decorated.renderCall !== "function")) {
      ...
      } else if (kind === "mcp" && (replaceExisting || typeof decorated.renderCall !== "function")) {
        decorated.renderCall = (args: unknown, theme: RenderTheme, context?: ToolRenderContextLike) => {
          ...
          return formatMcpCallLine(toolName, toolLabel, toRecord(args), theme, context);
        };
      }

      if (resolvedAdapter.renderResult && (replaceExisting || typeof decorated.renderResult !== "function")) {
        decorated.renderResult = resolvedAdapter.renderResult;
      } else if (kind === "read" && (replaceExisting || typeof decorated.renderResult !== "function")) {
      ...
      } else if (kind === "edit" && (replaceExisting || typeof decorated.renderResult !== "function")) {
      ...
      } else if (kind === "mcp" && (replaceExisting || typeof decorated.renderResult !== "function")) {
        decorated.renderResult = (result: ToolRenderInput, options: ToolRenderResultOptions, theme: RenderTheme, context?: ToolRenderContextLike) =>
          renderMcpResult(result, options, getConfig(), theme, context);
      }

      if (kind === "mcp" || (kind === "edit" && (overrideExisting || typeof decorated.renderShell !== "string"))) {
...
export function registerToolDisplayOverrides(
  ...
  installNativeMcpToolExecutionPatch(getConfig);
  registerCleanup(() => {
