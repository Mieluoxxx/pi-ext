...
const BASH_MAX_CHAR_VALUES = ["0", "1000", "2000", "4000", "8000", "16000"] as const;
...
function summarizeConfig(config: ToolDisplayConfig, capabilities: ToolDisplayCapabilities): string {
	...
	const parts = [
		...
		`bashChars=${config.bashCollapsedMaxChars}`,
...
function buildAdvancedNotes(
	...
	const notes = [
		...extra,
		"Manual JSON edits also expose registerToolOverrides.*, bashCollapsedMaxChars, expandedPreviewMaxLines, diffSplitMinWidth, diffCollapsedLines, diffIndicatorMode, and diffWordWrap.",
...
function buildInspectorSettings(
	...
	items.push(
		...
		{
			id: "bashCollapsedMaxChars",
			label: "Bash collapsed characters",
			currentValue: String(config.bashCollapsedMaxChars),
			values: BASH_MAX_CHAR_VALUES,
			inspectorTitle: "Bash Collapsed Characters",
			inspectorSummary: [
				"Sets the maximum visible character budget for unexpanded Bash output.",
				"The character and line budgets both apply; the first reached limit collapses the output. Set 0 to disable the character limit.",
			],
			inspectorOptions: [
				"0 — no collapsed character limit",
				"1000/2000/4000/8000/16000 — increasingly larger inline Bash output budgets",
			],
			inspectorAdvanced: buildAdvancedNotes(config, capabilities, [
				"This setting applies only to unexpanded Bash output and does not limit expanded output.",
			]),
			inspectorPath: configPath,
			searchTerms: ["bash", "collapsed", "characters", "char", "stdout", "limit"],
		},
		{
			id: "diffViewMode",
...
export function applyToolDisplaySetting(
	config: ToolDisplayConfig,
	id: string,
	value: string,
): ToolDisplayConfig {
	switch (id) {
		...
		case "bashCollapsedMaxChars":
			return {
				...config,
				bashCollapsedMaxChars: parseNumber(value, config.bashCollapsedMaxChars),
			};
		case "diffViewMode":
			return {
				...config,
			...
			const inspector = new SplitPaneInspectorModal(
				{
					...
					onChange: (id, newValue) => {
						const next = applyToolDisplaySetting(controller.getConfig(), id, newValue);
