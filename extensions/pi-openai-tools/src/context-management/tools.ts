...
export class ContextManagementToolController {
	...
	sync(active: boolean): boolean {
		...
		try {
			...
			// Pi initially activates registered tools; that is not an explicit Remote opt-in.
			const inactive = current.filter((name) => !this.registeredNames.has(name));
			const next = active ? [...new Set([...current, ...this.registeredNames])] : inactive;
			if (next.length !== current.length) api.setActiveTools(next);
			const actual = new Set(api.getActiveTools());
			if (![...this.registeredNames].every((name) => actual.has(name) === active)) {
				// A restrictive host policy must not leave a partially usable Remote runtime.
				api.setActiveTools(inactive);
				this.registrationValid = false;
				return false;
			}
			return true;
