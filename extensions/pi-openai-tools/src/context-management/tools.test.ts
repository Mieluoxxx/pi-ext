...
import { ContextManagementToolController, createContextManagementTools, NEW_CONTEXT_CHECKPOINT_REQUIRED_MESSAGE } from "./tools";
...
test("inactive Remote tools are hidden even when Pi initially activates every registered tool", () => {
	let active = ["read"];
	const registered: Array<{ name: string }> = [];
	const pi = {
		registerTool: (tool: { name: string }) => { registered.push(tool); active.push(tool.name); },
		getAllTools: () => registered,
		getActiveTools: () => active,
		setActiveTools: (names: string[]) => { active = names; },
	} as unknown as ExtensionAPI;
	const controller = new ContextManagementToolController(pi);
	controller.register(createContextManagementTools(pi, new CodexContextWindowManager(async () => undefined), () => true));
	expect(controller.sync(false)).toBe(true);
	expect(active).toEqual(["read"]);
	expect(controller.sync(true)).toBe(true);
	expect(active).toEqual(["read", ...registered.map((tool) => tool.name)]);
	controller.reset();
	expect(active).toEqual(["read"]);
});

test("Remote sync respects tool allowlists and rechecks ownership after registration", () => {
	let active = ["read"];
	let registered: Array<{ name: string }> = [];
	let rejectNotes = true;
	const pi = {
		registerTool: (tool: { name: string }) => registered.push(tool),
		getAllTools: () => registered,
		getActiveTools: () => active,
		setActiveTools: (names: string[]) => { active = rejectNotes ? names.filter((name) => name !== "notes") : names; },
	} as unknown as ExtensionAPI;
	const controller = new ContextManagementToolController(pi);
	controller.register(createContextManagementTools(pi, new CodexContextWindowManager(async () => undefined), () => true));
	...
	rejectNotes = false;
	expect(controller.sync(true)).toBe(true);
	registered = registered.map((tool) => tool.name === "history" ? { name: "history" } : tool);
