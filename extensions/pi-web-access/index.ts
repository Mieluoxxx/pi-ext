...
import { normalizeGetSearchContentParams, type LegacyGetSearchContentParams } from "./get-search-content-params.ts";
...
const DEFAULT_TOOL_NAMES: ToolNames = {
	...
	findSearchContent: "find_search_content",
};
...
type ResolvedStoredContent = {
	source: string;
	prefix: string;
	details: Record<string, unknown>;
};

type StoredContentError = { error: string; details: Record<string, unknown> };

function resolveStoredContent(data: StoredSearchData, responseId: string, contentIndex?: number): ResolvedStoredContent | StoredContentError {
	if (data.type === "research") {
		const artifact = getResearchArtifact(responseId);
		if (!artifact) return { error: "Artifact not found", details: { error: "Artifact not found", responseId } };
		if (contentIndex !== undefined && contentIndex !== 0) {
			return { error: `Content index ${contentIndex} is invalid for a single research artifact. Use contentIndex 0 or omit it.`, details: { error: "Index out of range", contentIndex } };
		}
		const source = JSON.stringify(artifact, null, 2);
		return { source, prefix: "", details: { responseId, type: "research", contentLength: source.length, contentIndex: 0 } };
	}
	...
	if (data.type === "search" && data.queries) {
		...
		if (!queryData) {
			return {
				error: `Content index ${index} is out of range. Available content indexes: ${data.queries.map((query, i) => `[${i}] "${query.query}"`).join("\n") || "none"}`,
				details: { error: "Index out of range", contentIndex: index },
			};
		}
		if (queryData.error) return { error: queryData.error, details: { error: queryData.error, query: queryData.query, contentIndex: index } };
		return {
			source: formatFullResults(queryData),
			prefix: "",
			details: { responseId, type: "search", contentIndex: index, query: queryData.query, resultCount: queryData.results.length },
		};
	}
	...
	if (data.type === "fetch" && data.urls) {
		...
		if (!urlData) {
			return {
				error: `Content index ${index} is out of range. Available content indexes: ${data.urls.map((url, i) => `[${i}] ${url.title || url.url}`).join("\n") || "none"}`,
				details: { error: "Index out of range", contentIndex: index },
			};
		}
		if (urlData.error) return { error: urlData.error, details: { error: urlData.error, url: urlData.url, contentIndex: index } };
		return {
			source: urlData.content,
			prefix: `# ${urlData.title || urlData.url}\n\n`,
			details: { responseId, type: "fetch", contentIndex: index, url: urlData.url, title: urlData.title, contentLength: urlData.content.length },
		};
	}

	return { error: "Invalid data", details: { error: "Invalid data" } };
}
...
function formatSourceCheckResult(artifact: ResearchArtifact, retrievalTools: string[] = [DEFAULT_TOOL_NAMES.getSearchContent]): string {
	...
	lines.push(retrievalTools.length > 0
		? `Artifact responseId: ${artifact.id} (retrievable via ${joinToolNames(retrievalTools)}).`
...
export default function (pi: ExtensionAPI) {
	...
	const findSearchContentEnabled = isToolEnabled(initConfig, "findSearchContent");
	...
	const contentRetrievalTools = [
		...(getSearchContentEnabled ? [toolNames.getSearchContent] : []),
		...(findSearchContentEnabled ? [toolNames.findSearchContent] : []),
	];
	const findContentHint = findSearchContentEnabled
		? ` Use ${toolNames.findSearchContent} to locate passages instead of paging manually.`
		: "";
	const getContentHint = getSearchContentEnabled
		? ` Use ${toolNames.getSearchContent} only to read or continue a truncated slice.`
		: "";
	const fetchContentStorageNote = contentRetrievalTools.length > 0
		? `Full original content is stored for retrieval with ${joinToolNames(contentRetrievalTools)}.`
	...
	function buildSearchReturn(opts: SearchReturnOptions): AgentToolResult<Record<string, unknown>> {
		...
		if (opts.results.length > 1) {
			output += `\n---\nStored content indexes for ${searchId}:\n${opts.results.map((result, index) => `[contentIndex: ${index}] "${result.query}"`).join("\n")}`;
		}
	...
	if (sourceCheckEnabled) pi.registerTool({
		...
		async execute(_callId, params, signal, _onUpdate, ctx) {
			...
			return {
			content: [{ type: "text", text: formatSourceCheckResult(artifact, contentRetrievalTools) }],
	...
	if (fetchContentEnabled) pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx): Promise<AgentToolResult<Record<string, unknown>>> {
				...
				if (truncated) {
					...
					output += storedContent
						? getSearchContentEnabled
							? `Use ${toolNames.getSearchContent}({ responseId: "${responseId}", contentIndex: 0, offset: ${slice.endOffset} }) for the next slice.`
							: findSearchContentEnabled
								? `Use ${toolNames.findSearchContent} with responseId "${responseId}" and findText to locate a passage.`
			...
			for (const [index, { url, title, content, error }] of presentedResults.entries()) {
				if (error) {
					output += `- [contentIndex: ${index}] ${url}: Error - ${error}\n`;
				} else {
					output += `- [contentIndex: ${index}] ${title || url} (${content.length} chars)\n`;
				}
			}
			output += storedContent
				? getSearchContentEnabled
					? `\n---\nUse ${toolNames.getSearchContent}({ responseId: "${responseId}", contentIndex: 0 }) to retrieve bounded content slices.`
					: findSearchContentEnabled
						? `\n---\nUse ${toolNames.findSearchContent} with responseId "${responseId}" and findText to locate passages.`
						: "\n---\nContent retrieval is not registered."
	...
	if (getSearchContentEnabled) {
		...
		pi.registerTool({
			...
			description: `Read bounded content slices from a previous ${storedContentSources} call.`,
			promptSnippet:
			`Use ${toolNames.getSearchContent} only to read or continue a truncated slice. Pass responseId and contentIndex (omit contentIndex when there is only one item).${findContentHint}`,
			parameters: Type.Object({
				responseId: Type.String({ description: `The responseId from ${storedContentSources}` }),
				contentIndex: Type.Optional(Type.Integer({ minimum: 0, description: "Index of the stored URL or query. Omit when there is only one." })),
			offset: Type.Optional(Type.Integer({ minimum: 0, description: "Character offset for fetched URL content slices (default 0). Use the nextOffset returned by the previous call to continue." })),
		...
		async execute(_toolCallId, rawParams): Promise<AgentToolResult<Record<string, unknown>>> {
			let params = normalizeGetSearchContentParams(rawParams as LegacyGetSearchContentParams);
			...
			const hasLegacySelector = params.query !== undefined || params.queryIndex !== undefined || params.url !== undefined || params.urlIndex !== undefined;
			if (!hasLegacySelector) {
				const resolved = resolveStoredContent(data, params.responseId, params.contentIndex);
				if ("error" in resolved) return { content: [{ type: "text", text: resolved.error }], details: resolved.details };
				const offset = params.offset ?? 0;
				const limit = params.limit ?? maxInlineContentChars;
				if (!Number.isInteger(offset) || offset < 0) {
					return {
						content: [{ type: "text", text: `Invalid offset: received ${formatInputValue(offset)}; offset must be a non-negative integer. Use 0 or a larger integer.` }],
						details: { error: "Invalid offset", offset },
					};
				}
				if (!Number.isInteger(limit) || limit <= 0 || limit > maxInlineContentChars) {
					return {
						content: [{ type: "text", text: `Invalid limit: received ${formatInputValue(limit)}; limit must be an integer from 1 to ${maxInlineContentChars}. Use a value in that range.` }],
				...
				const endOffset = Math.min(offset + limit, resolved.source.length);
				const contentSlice = resolved.source.slice(offset, endOffset);
				const hasMore = endOffset < resolved.source.length;
				let text = `${resolved.prefix}${contentSlice}`;
				if (hasMore || offset > 0) {
					text += `\n\n---\nShowing chars ${offset}-${endOffset} of ${resolved.source.length}.`;
					if (hasMore) text += ` Use ${toolNames.getSearchContent}({ responseId: "${params.responseId}", contentIndex: ${resolved.details.contentIndex}, offset: ${endOffset}, limit: ${limit} }) for the next slice.`;
				}
				return {
					content: [{ type: "text", text }],
					details: { ...resolved.details, offset, limit, returnedChars: contentSlice.length, nextOffset: hasMore ? endOffset : null, truncated: hasMore },
				};
			}
			if (params.contentIndex !== undefined && params.query === undefined && params.queryIndex === undefined && params.url === undefined && params.urlIndex === undefined) {
				if (data.type === "search" && data.queries) params = { ...params, queryIndex: params.contentIndex };
				if (data.type === "fetch" && data.urls) params = { ...params, urlIndex: params.contentIndex };
			} else if (params.contentIndex === undefined && params.query === undefined && params.queryIndex === undefined && params.url === undefined && params.urlIndex === undefined) {
				if (data.type === "search" && data.queries?.length === 1) params = { ...params, queryIndex: 0 };
				if (data.type === "fetch" && data.urls?.length === 1) params = { ...params, urlIndex: 0 };
			}
			...
			if (data.type === "fetch" && data.urls) {
				...
				if (hasMore || offset > 0) {
					...
					if (hasMore) {
						text += ` Use ${toolNames.getSearchContent}({ responseId: "${params.responseId}", contentIndex: ${selectedUrlIndex}, offset: ${endOffset}, limit: ${limit} }) for the next slice.`;
					}
				}
		...
		renderCall(args, theme) {
			const { responseId, contentIndex, query, queryIndex, url, urlIndex, offset, findText } = args as {
				responseId: string;
				contentIndex?: number;
				query?: string;
				queryIndex?: number;
			...
			let target = "";
			if (contentIndex !== undefined) target = `contentIndex=${contentIndex}`;
			else if (query) target = `query="${query}"`;
	...
	if (findSearchContentEnabled) {
		pi.registerTool({
			name: toolNames.findSearchContent,
			label: "Find Search Content",
			description: `Find matching passages in content stored by ${storedContentSources}.`,
			promptSnippet: `Use ${toolNames.findSearchContent} to locate passages in stored content. Pass the previous responseId and contentIndex when there are multiple stored items.${getContentHint}`,
			parameters: Type.Object({
				responseId: Type.String({ description: `The responseId from ${storedContentSources}` }),
				contentIndex: Type.Optional(Type.Integer({ minimum: 0, description: "Index of the stored URL or query. Omit when there is only one." })),
				findText: Type.Union([
				...
				], { description: "Text or texts to find. Returns bounded matching passages with nearby context." }),
				findMode: Type.Optional(StringEnum(["exact", "case-insensitive", "fuzzy"], { description: "Matching mode (default: case-insensitive)." })),
			}),
			async execute(_toolCallId, params): Promise<AgentToolResult<Record<string, unknown>>> {
				...
				const resolved = resolveStoredContent(data, params.responseId, params.contentIndex);
				if ("error" in resolved) return { content: [{ type: "text", text: resolved.error }], details: resolved.details };
				const { source, prefix, details } = resolved;

				try {
					const found = findContent(source, normalizeFindQueries(params.findText), (params.findMode ?? "case-insensitive") as FindMode);
					const { text, ...findDetails } = found;
					return {
						content: [{ type: "text", text: `${prefix}${text}` }],
						details: { ...details, findMode: params.findMode ?? "case-insensitive", ...findDetails },
					};
				} catch (err) {
					const error = err instanceof Error ? err.message : String(err);
					return {
						content: [{ type: "text", text: `Unable to find ${formatInputValue(params.findText)} in responseId ${formatInputValue(params.responseId)}: ${error}. Check findText and use a supported findMode.` }],
						details: { ...details, error },
					};
				}
			},
			renderCall(args, theme) {
				const params = args as { responseId: string; contentIndex?: number; findText: string | string[] };
				const count = Array.isArray(params.findText) ? params.findText.length : 1;
				const index = params.contentIndex === undefined ? "" : ` @ ${params.contentIndex}`;
				return new Text(theme.fg("toolTitle", theme.bold("find_content ")) + theme.fg("accent", `${params.responseId.slice(0, 8)}${index} · ${count} terms`), 0, 0);
			},
			renderResult(result, { expanded }, theme) {
				const details = result.details as { error?: string; matchCount?: number; returnedMatches?: number };
				if (details?.error) return new Text(theme.fg("error", `Error: ${details.error}`), 0, 0);
				const status = `${details?.matchCount ?? 0} matches, ${details?.returnedMatches ?? 0} shown`;
				if (!expanded) return new Text(theme.fg("success", status), 0, 0);
				const text = result.content.find((content) => content.type === "text")?.text || "";
				return new Text(theme.fg("success", status) + "\n" + theme.fg("dim", text.slice(0, 500)), 0, 0);
			},
		});
	}
