...
This is the `@moguw/pi-web-access` fork maintained in [Mieluoxxx/pi-ext](https://github.com/Mieluoxxx/pi-ext/tree/main/extensions/pi-web-access), imported from [nicobailon/pi-web-access](https://github.com/nicobailon/pi-web-access) at `597be04626732e07fbef5632ac834deea745a0a8` (v0.24.2), including local tool-registration and content-retrieval changes. The upstream MIT license and attribution are preserved.
...
[![npm version](https://img.shields.io/npm/v/@moguw/pi-web-access?style=for-the-badge)](https://www.npmjs.com/package/@moguw/pi-web-access)
...
From the `pi-ext` repository root (no npm publication required):
...
### get_search_content and find_search_content

Retrieve stored content from previous searches or fetches. Fetched URL content is stored in full in a private `web-search-cache` directory under the Pi config directory, not in the session JSONL. This includes `fetch_content` answer mode, which stores the original page content. The cache has a one-hour lifetime and fixed limits of 128 entries and 128 MiB; when either limit is reached, the oldest entries are removed first. On macOS and Linux the cache directory and files are kept at permissions `0700` and `0600`, respectively.

```typescript
get_search_content({ responseId: "abc123", contentIndex: 0 })
get_search_content({ responseId: "abc123", contentIndex: 0, offset: 30000 })
find_search_content({ responseId: "abc123", contentIndex: 0, findText: "installation" })
find_search_content({ responseId: "abc123", contentIndex: 0, findText: ["timeout", "retry"], findMode: "fuzzy" })
```

Use `find_search_content` to locate passages and `get_search_content` only to read or continue a truncated slice. `contentIndex` identifies a stored URL or query when a response contains multiple items; omit it when there is only one. `findMode` supports `exact`, `case-insensitive` (default), and `fuzzy`. Finder output is capped at 20,000 characters with match counts and nearby context. The default `limit` and maximum permitted `limit` use `maxInlineContentChars`.
...
```json
{
  ...
  "tools": {
    ...
    "findSearchContent": { "enabled": true }
  },
  "commands": {
    "websearch": { "enabled": true },
