...
test("malformed config falls back during extension registration", () => {
	...
	assert.deepEqual(registered.tools.map(tool => tool.name), ["web_search", "source_check", "fetch_content", "get_search_content", "find_search_content"]);
});
...
test("tool registration gates support legacy and per-tool config", () => {
	assert.deepEqual(registeredToolNames({ webSearch: { enabled: false } }), ["fetch_content", "get_search_content", "find_search_content"]);
	assert.deepEqual(registeredToolNames({
		webSearch: { enabled: false },
		tools: { webSearch: { enabled: true }, sourceCheck: { enabled: true }, fetchContent: { enabled: false } },
	}), ["web_search", "source_check", "get_search_content", "find_search_content"]);
	assert.deepEqual(registeredToolNames({
		tools: { sourceCheck: { enabled: false }, getSearchContent: { enabled: false } },
	}), ["web_search", "fetch_content", "find_search_content"]);
});
...
test("registered tools do not advertise disabled content retrieval tools", () => {
	const fetchTool = registeredTool({ tools: {
		getSearchContent: { enabled: false },
		findSearchContent: { enabled: false },
	} }, "fetch_content");
	assert.ok(fetchTool);
...
test("retrieval prompts only mention tools that are registered", () => {
	const getTool = registeredTool({ tools: { findSearchContent: { enabled: false } } }, "get_search_content");
	assert.doesNotMatch(getTool.promptSnippet, /find_search_content/);

	const findTool = registeredTool({ tools: { getSearchContent: { enabled: false } } }, "find_search_content");
	assert.doesNotMatch(findTool.promptSnippet, /get_search_content/);
});
...
test("tool names can be configured without changing defaults", () => {
	assert.deepEqual(registeredToolNames({}), ["web_search", "source_check", "fetch_content", "get_search_content", "find_search_content"]);
	assert.deepEqual(registeredToolNames({
		toolNames: {
			...
			findSearchContent: "find_content",
		},
	}), ["research_web", "verify_sources", "grab_content", "open_content", "find_content"]);
});
...
test("webSearch.enabled false registers only fetch tools and ignores disabled-name duplicates", () => {
	...
	}), ["grab_content", "open_content", "find_content"]);
