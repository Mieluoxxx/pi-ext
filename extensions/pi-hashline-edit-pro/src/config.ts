...
function parseConfig(content: string): Config {
  ...
  const disabledTools = isRec(parsed) && Array.isArray(parsed.disabledTools)
    ? parsed.disabledTools.filter((tool): tool is string => typeof tool === "string")
    : undefined;
  return disabledTools?.length ? { autoRead, disabledTools } : { autoRead };
}
