# @moguw/pi-hashline-edit-pro

[![npm version](https://img.shields.io/npm/v/@moguw/pi-hashline-edit-pro.svg)](https://www.npmjs.com/package/@moguw/pi-hashline-edit-pro) [![npm downloads](https://img.shields.io/npm/dm/@moguw/pi-hashline-edit-pro.svg)](https://www.npmjs.com/package/@moguw/pi-hashline-edit-pro)

Maintained in [Mieluoxxx/pi-ext](https://github.com/Mieluoxxx/pi-ext/tree/main/extensions/pi-hashline-edit-pro), based on [YuGiMob/pi-hashline-edit-pro](https://github.com/YuGiMob/pi-hashline-edit-pro) at `77d545e` (v2.7.2), with the local `disabledTools` configuration changes. The upstream MIT license and existing config/storage paths are preserved.
...
`disabledTools` (optional) skips registration for the listed tools entirely, which is useful when another extension overrides a built-in tool name that this extension also registers (for example an enhanced `grep`): pi treats duplicate tool names across extensions as a fatal load error, so disabling the duplicate here keeps both extensions loadable. Valid names: `read`, `replace`, `insert`, `grep`, `undo_last_change`. Takes effect on restart.
