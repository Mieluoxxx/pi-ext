...
CHOOSE AT MOST ONE SESSION LIFECYCLE SELECTOR BEFORE CALLING: \`command\`, a non-empty \`spawn\`, \`sessionId\`, or \`attach\`.
- Start a new shell session: use exactly one of \`command\` or a non-empty \`spawn\`. Do not include \`sessionId\` or \`attach\`.
- Operate an existing session: use \`sessionId\` with query, \`input\`, \`kill\`, \`background\`, or settings fields. Do not include \`command\`, \`spawn\`, or \`attach\`.
- Reattach a background session: use \`attach\` as the selector, with an optional \`mode\`. Do not include \`command\`, \`spawn\`, or \`sessionId\`.
- List or dismiss background sessions: use \`listBackground\` or \`dismissBackground\` only. Do not include \`command\`, \`spawn\`, \`sessionId\`, or \`attach\`.
...
- Send a slash command: \`interactive_shell({ sessionId: "shell-1", input: "/compact", submit: true })\`
- Query status/output: \`interactive_shell({ sessionId: "shell-1" })\`
- End a session: \`interactive_shell({ sessionId: "shell-1", kill: true })\`

If a tool call returns an error, it did not perform the requested operation. Do not claim success. Do not repeat the unchanged invalid call; rebuild it from the matching minimal call above.
...
export const toolParameters = Type.Object({
	command: Type.Optional(
		Type.String({
			description: "The raw CLI command to start a NEW session (e.g., 'pi \"Fix the bug\"'). Mutually exclusive with spawn, sessionId, and attach. Omit it for existing-session operations.",
		}),
	),
	spawn: Type.Optional(
		Type.Object({
			...
			description: "Structured request to start a NEW session. Mutually exclusive with command, sessionId, and attach. Omit it for existing-session operations such as input, status, kill, or background.",
		}),
	),
	sessionId: Type.Optional(
		Type.String({
			description: "Target an existing session for query, input, kill, background, or settings. Do not combine with command, spawn, or attach.",
		}),
	),
	kill: Type.Optional(
		Type.Boolean({
	...
	attach: Type.Optional(
		Type.String({
			description: "Background session ID to reattach. Opens an overlay with the specified mode. Do not combine with command, spawn, or sessionId.",
		}),
	),
