...
## Session Lifecycle Selector

Choose at most one session lifecycle selector per `interactive_shell` call: `command`, a non-empty `spawn`, `sessionId`, or `attach`.

- Start a new shell session: use `command` or a non-empty `spawn`.
- Operate an existing session: use `sessionId` with input, query, kill, background, or settings fields.
- Reattach a background session: use `attach` as the selector, with an optional `mode`.
- List or dismiss background sessions: use `listBackground` or `dismissBackground`.

For existing sessions, omit `command`, `spawn`, and `attach`. Use `submit: true` when sending slash commands or prompts. An error means the action did not run: do not report success and do not repeat the unchanged invalid call; rebuild it from the matching minimal example.
